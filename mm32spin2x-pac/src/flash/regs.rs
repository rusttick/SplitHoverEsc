...
impl defmt::Format for Acr {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Acr {{ latency: {=u8:?}, hlfcya: {=bool:?}, prftbe: {=bool:?} }}",
            self.latency(),
            self.hlfcya(),
            self.prftbe()
        )
...
impl Keyr {
    ...
    #[doc = "Flash key"]
    #[inline(always)]
    pub const fn set_fkeyr(&mut self, val: u32) {
        self.0 = (self.0 & !(0xffff_ffff << 0usize)) | (((val as u32) & 0xffff_ffff) << 0usize);
    }
}
...
impl defmt::Format for Keyr {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(f, "Keyr {{ fkeyr: {=u32:?} }}", self.fkeyr())
    }
}
...
impl defmt::Format for Obr {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Obr {{ opterr: {=bool:?}, wdg_sw: {=bool:?}, n_rst_stop: {=bool:?}, n_rst_stdby: {=bool:?}, n_boot1: {=bool:?}, data0: {=u8:?}, data1: {=u8:?} }}",
            self.opterr(),
            self.wdg_sw(),
            self.n_rst_stop(),
            self.n_rst_stdby(),
