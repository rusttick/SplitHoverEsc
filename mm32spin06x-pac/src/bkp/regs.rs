...
impl Cr {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn tpal(&self) -> bool {
        let val = (self.0 >> 1usize) & 0x01;
        val != 0
    }
    #[doc = "TAMPER pin active level"]
    #[inline(always)]
    pub const fn set_tpal(&mut self, val: bool) {
...
impl Rtccr {
    ...
    #[doc = "Alarm or second output selection"]
    #[inline(always)]
    pub const fn set_asos(&mut self, val: bool) {
        self.0 = (self.0 & !(0x01 << 9usize)) | (((val as u16) & 0x01) << 9usize);
    }
}
...
impl defmt::Format for Rtccr {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Rtccr {{ cal: {=u8:?}, cco: {=bool:?}, asoe: {=bool:?}, asos: {=bool:?} }}",
            self.cal(),
            self.cco(),
            self.asoe(),
            self.asos()
